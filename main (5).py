...
class FTPManager:
    ...
    def read_csv(self):
        try:
            ...
            ftp.quit()
            # utf-8-sig handles BOM (from Excel/Windows); fallback to Windows-1256
            raw = buf.getvalue()
            try:
                text = raw.decode("utf-8-sig")
            except UnicodeDecodeError:
                try:
                    text = raw.decode("windows-1256")
                except UnicodeDecodeError:
    ...
    def write_csv(self, records, fieldnames):
        try:
            ...
            ftp.storbinary(f"STOR {self.cfg['filename']}",
                           io.BytesIO(data.encode("utf-8-sig")))  # BOM for Excel compatibility
            ftp.quit()
            return True
        except Exception as e:
            print(f"[FTP Write] {e}")
...
class ContactApp(MDApp):
    ...
    def _snack(self, text):
        # Only call ar() if text contains original Arabic (0600-06FF)
        # Prevents double-processing when text is already ar()-processed
        has_orig_ar = any('\u0600' <= c <= '\u06FF' for c in (text or ''))
        msg = ar(text) if has_orig_ar else str(text)
        def _show(dt):
            try:
                from kivy.core.window import Window
                from kivymd.uix.snackbar import MDSnackbar, MDSnackbarText
                # text_size REQUIRED for halign to work in Kivy labels
                snack_w = Window.width * 0.9 - dp(16)
                lbl = MDSnackbarText(
                    text=msg, halign="right",
                    text_size=(snack_w, None))
                MDSnackbar(lbl, y=dp(24), pos_hint={"center_x": .5},
                           size_hint_x=.9, duration=3).open()
