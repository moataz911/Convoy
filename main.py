...
try:
    ...
    print("تحذير: مكتبات دعم العربية غير متوفرة. قد يظهر النص العربي بشكل خاطئ.")

def fix_arabic_text(text):
    """
    معالجة النص العربي لعرضه بشكل صحيح (RTL + إعادة تشكيل)
    """
...
def is_rtl_text(text):
    ...
    return arabic_chars > len(text) * 0.3  # إذا كان 30% من النص عربي

# ─── دوال مساعدة لإنشاء widgets تدعم العربية ───────────────
def create_arabic_label(text, **kwargs):
    """
    إنشاء Label يعرض النص العربي بشكل صحيح
    """
    display_text = fix_arabic_text(text) if text else ""
    label = MDLabel(text=display_text, **kwargs)
    return label

def create_arabic_textfield(hint_text="", text="", **kwargs):
    """
    إنشاء TextField محسّن للعربية
    - hint_text: نص التلميح
    - text: القيمة الافتراضية
    """
    # معالجة نص التلميح للعرض الصحيح
    display_hint = fix_arabic_text(hint_text) if hint_text else ""
    
    # إنشاء TextField
    tf = MDTextField(hint_text=display_hint, text=text, **kwargs)
    
    # ضبط الاتجاه للعربية إذا كان النص عربياً
...
def format_display_text(text):
    """
    تنسيق النص للعرض (Labels, list items, etc)
    """
...
class ContactApp(MDApp):
    ...
    def build(self):
        ...
        # Process Arabic text in KV string
        import re
        kv_processed = KV
        # Find all Arabic text in quotes and apply fix_arabic_text()
        arabic_pattern = r'text:\s*["\']([^"\']*[\u0600-\u06FF][^"\']*)["\']'
        def replace_arabic(match):
            text = match.group(1)
            # Skip if it's a property binding or callback
            if '{' in text or 'root.' in text or 'app.' in text:
                return match.group(0)
            fixed = fix_arabic_text(text)
            return f'text: "{fixed}"'
        kv_processed = re.sub(arabic_pattern, replace_arabic, kv_processed)
        return Builder.load_string(kv_processed)
    ...
    def _snack(self, text: str):
        def _show(dt):
            try:
                ...
                MDSnackbar(MDSnackbarText(text=fix_arabic_text(text)),
                           y=dp(24), pos_hint={"center_x": 0.5},
                           size_hint_x=0.9, duration=3).open()
            except (ImportError, TypeError):
                try:
                    from kivymd.uix.snackbar import Snackbar
                    s = Snackbar(); s.text = fix_arabic_text(text); s.open()
                except Exception:
    ...
    def _simple_dialog(self, title, text):
        try:
            d = MDDialog(title=fix_arabic_text(title), text=fix_arabic_text(text),
                         buttons=[MDFlatButton(text=fix_arabic_text("حسناً"), on_release=lambda x: d.dismiss())])
            d.open()
    ...
    def _refresh_status_bars(self):
        try:
            ...
            lbl.text = "" if self._ftp_ok else fix_arabic_text("غير متصل بسيرفر FTP")
        except: pass
    ...
    def _show_no_ftp_dialog(self, msg=""):
        try:
            d = MDDialog(
                title=fix_arabic_text("غير متصل بالسيرفر"),
                text=fix_arabic_text(f"لا يمكن حفظ البيانات.\nيجب الاتصال بسيرفر FTP اولاً.\n\n{msg}"),
                buttons=[
                    MDRaisedButton(text=fix_arabic_text("اعدادات FTP"),
                                   on_release=lambda x: (d.dismiss(), self.go_to("settings"))),
                    MDFlatButton(text=fix_arabic_text("اغلاق"), on_release=lambda x: d.dismiss()),
                ])
            d.open()
    ...
    def _refresh_list(self, append=False):
        ...
        if not self.records:
            if not append:
                lst.add_widget(MDLabel(text=fix_arabic_text("لا توجد سجلات"), halign="center",
    ...
    def _make_item(self, i, rec):
        ...
        info.add_widget(MDLabel(text=fix_arabic_text(rec.get("الاسم","بدون اسم")),
        ...
        if details:
            info.add_widget(MDLabel(text=fix_arabic_text(details), font_style="Caption",
        ...
        for fd in self.fields:
            if fd["type"] == "multiselect":
                ...
                if selected:
                    ...
                    chips_row.add_widget(MDLabel(text=fix_arabic_text(f'{fd["name"]}: '),
                    ...
                    for opt in selected:
                        try:
                            ...
                            chips_row.add_widget(MDLabel(text=fix_arabic_text(f"[{opt}]"),
                                                         font_style="Caption", adaptive_size=True))
    ...
    def _make_field_widget(self, fd, current_value):
        ...
        if fd["type"] == "text":
            tf = self.create_arabic_textfield(
            ...
            lbl = MDLabel(text=fix_arabic_text(fd["name"]), adaptive_height=True)
            ...
            container.add_widget(MDLabel(text=fix_arabic_text(fd["name"]), font_style="Subtitle1",
                                         adaptive_height=True))
            ...
            for opt in options:
                ...
                cb_row.add_widget(MDLabel(text=fix_arabic_text(opt), font_style="Caption", adaptive_size=True))
    ...
    def _confirm_delete(self, idx: int):
        ...
        d = MDDialog(
            text=fix_arabic_text("هل انت متاكد من حذف هذا السجل؟"),
            buttons=[
                MDFlatButton(text=fix_arabic_text("الغاء"), on_release=lambda x: d.dismiss()),
                MDRaisedButton(text=fix_arabic_text("حذف"), md_bg_color="red", on_release=_do),
            ])
        d.open()
        ...
        for rec in self.records:
            if any(q in str(v).lower() for v in rec.values()):
                item = TwoLineAvatarIconListItem(
                    text=fix_arabic_text(rec.get("الاسم","")),
                    secondary_text=fix_arabic_text(f"السن: {rec.get('السن','')} | الهاتف: {rec.get('رقم الهاتف','')}"),
                )
                lst.add_widget(item)
    ...
    def _refresh_fields_ui(self, *a):
        ...
        for fd in self.fields:
            ...
            item = TwoLineAvatarIconListItem(
                text=fix_arabic_text(name),
                secondary_text=fix_arabic_text(secondary + (" · أساسي" if name in BUILTIN else ""))
            )
