...
def fix_arabic(text):
    """إصلاح عرض النص العربي (Reshaping & Bidi) مع الحفاظ على الإنجليزية"""
...
<HomeScreen>:
    ...
    MDBoxLayout:
        ...
        MDTopAppBar:
            title: app.fix_text("مدير جهات الاتصال")
...
<AddScreen>:
    ...
    MDBoxLayout:
        ...
        MDBoxLayout:
            ...
            padding: dp(10)
            MDLabel:
                text: app.fix_text("غير متصل بسيرفر FTP - لا يمكن الحفظ")
                halign: "center"
                theme_text_color: "Custom"
                text_color: 1, 1, 1, 1
                font_style: "Caption"
...
<EditScreen>:
    ...
    MDBoxLayout:
        orientation: "vertical"
        MDTopAppBar:
            title: app.fix_text("تعديل السجل")
            left_action_items: [["arrow-right", lambda x: app.go_to("home")]]
            elevation: 4
        MDBoxLayout:
            id: ftp_warning_edit
        ...
        MDScrollView:
            MDBoxLayout:
                ...
                adaptive_height: True
                MDRaisedButton:
                    text: app.fix_text("تحديث البيانات")
                    pos_hint: {"center_x": 0.5}
                    size_hint_x: 0.8
...
<SearchScreen>:
    ...
    MDBoxLayout:
        ...
        MDBoxLayout:
            ...
            MDTextField:
                id: search_input
                hint_text: app.fix_text("ابحث بالاسم او اي معلومة...")
...
<SettingsScreen>:
    ...
    MDBoxLayout:
        orientation: "vertical"
        MDTopAppBar:
            title: app.fix_text("اعدادات FTP")
            left_action_items: [["arrow-right", lambda x: app.go_to("home")]]
            elevation: 4
        MDScrollView:
            MDBoxLayout:
                ...
                MDBoxLayout:
                    ...
                    spacing: dp(10)
                    MDRaisedButton:
                        text: app.fix_text("حفظ")
                        on_release: app.save_settings()
                    MDFlatButton:
                        text: app.fix_text("اختبار الاتصال")
...
<FieldsScreen>:
    ...
    MDBoxLayout:
        orientation: "vertical"
        MDTopAppBar:
            title: app.fix_text("ادارة الحقول")
            left_action_items: [["arrow-right", lambda x: app.go_to("home")]]
            elevation: 4
        MDBoxLayout:
            orientation: "vertical"
            ...
            MDBoxLayout:
                ...
                MDTextField:
                    id: new_field_input
                    hint_text: app.fix_text("اسم الحقل الجديد")
                    size_hint_x: 0.5
                MDTextField:
                    id: field_type_input
                    hint_text: app.fix_text("النوع: text/checkbox/multiselect")
                    size_hint_x: 0.5
            MDBoxLayout:
                adaptive_height: True
                spacing: dp(5)
                MDTextField:
                    ...
                    hint_text: app.fix_text("خيارات القائمة: خيار1,خيار2,خيار3")
                MDIconButton:
                    icon: "plus"
...
class ContactApp(MDApp):
    ...
    def build(self):
        ...
        self.theme_cls.font_styles.update({
            ...
            "Overline": [font_name, 10, False, 1.5],
        })
    ...
    def _snack(self, text: str):
        def _show(dt):
            try:
                ...
                MDSnackbar(MDSnackbarText(text=self.fix_text(text)),
    ...
    def _simple_dialog(self, title, text):
        try:
            d = MDDialog(title=self.fix_text(title), text=self.fix_text(text),
                         buttons=[MDFlatButton(text=self.fix_text("حسناً"), on_release=lambda x: d.dismiss())])
            d.open()
    ...
    def _refresh_status_bars(self):
        try:
            ...
            lbl.text = "" if self._ftp_ok else self.fix_text("غير متصل بسيرفر FTP")
        except: pass
    ...
    def _show_no_ftp_dialog(self, msg=""):
        try:
            d = MDDialog(
                title=self.fix_text("غير متصل بالسيرفر"),
                text=self.fix_text(f"لا يمكن حفظ البيانات.\nيجب الاتصال بسيرفر FTP اولاً.\n\n{msg}"),
                buttons=[
                    MDRaisedButton(text=self.fix_text("اعدادات FTP"),
                                   on_release=lambda x: (d.dismiss(), self.go_to("settings"))),
                    MDFlatButton(text=self.fix_text("اغلاق"), on_release=lambda x: d.dismiss()),
                ])
            d.open()
    ...
    def _refresh_list(self, append=False):
        ...
        if not self.records:
            if not append:
                lst.add_widget(MDLabel(text=self.fix_text("لا توجد سجلات"), halign="center",
    ...
    def _make_item(self, i, rec):
        ...
        name_text = rec.get("الاسم","بدون اسم")
        info.add_widget(MDLabel(text=self.fix_text(name_text),
        ...
        if details:
            info.add_widget(MDLabel(text=self.fix_text(details), font_style="Caption",
                                    theme_text_color="Secondary", adaptive_height=True, halign="right"))
        row.add_widget(info)
        ...
        for fd in self.fields:
            if fd["type"] == "multiselect":
                ...
                if selected:
                    ...
                    chips_row.add_widget(MDLabel(text=self.fix_text(f'{fd["name"]}: '),
                                                 font_style="Caption", adaptive_size=True,
                                                 theme_text_color="Secondary"))
                    for opt in selected:
                        try:
                            chip = MDChip(text=self.fix_text(opt))
                            chips_row.add_widget(chip)
                        except Exception:
                            chips_row.add_widget(MDLabel(text=self.fix_text(f"[{opt}]"),
                                                         font_style="Caption", adaptive_size=True))
    ...
    def _make_field_widget(self, fd, current_value):
        if fd["type"] == "text":
            tf = MDTextField(
                hint_text=self.fix_text(fd["name"] + (" (مطلوب)" if fd.get("required") else "")),
                mode="rectangle",
            ...
            lbl = MDLabel(text=self.fix_text(fd["name"]), adaptive_height=True, halign="right")
            ...
            container.add_widget(MDLabel(text=self.fix_text(fd["name"]), font_style="Subtitle1",
            ...
            for opt in options:
                ...
                cb_row.add_widget(MDLabel(text=self.fix_text(opt), font_style="Caption", adaptive_size=True))
    ...
    def _confirm_delete(self, idx: int):
        ...
        d = MDDialog(
            text=self.fix_text("هل انت متاكد من حذف هذا السجل؟"),
            buttons=[
                MDFlatButton(text=self.fix_text("الغاء"), on_release=lambda x: d.dismiss()),
                MDRaisedButton(text=self.fix_text("حذف"), md_bg_color="red", on_release=_do),
            ])
        d.open()
        ...
        for rec in self.records:
            if any(q in str(v).lower() for v in rec.values()):
                item = TwoLineAvatarIconListItem(
                    text=self.fix_text(rec.get("الاسم","")),
                    secondary_text=self.fix_text(f"السن: {rec.get('السن','')} | الهاتف: {rec.get('رقم الهاتف','')}"),
                )
                lst.add_widget(item)
    ...
    def _refresh_fields_ui(self, *a):
        ...
        for fd in self.fields:
            ...
            tp   = fd.get("type","text")
            item = TwoLineAvatarIconListItem(
                text=self.fix_text(name),
                secondary_text=self.fix_text(f"النوع: {tp}"),
            )
            if name not in BUILTIN:
                btn = IconRightWidget(icon="delete", theme_text_color="Error")
                btn.bind(on_release=lambda x, n=name: self.delete_field(n))
                item.add_widget(btn)
            lst.add_widget(item)

    def add_field(self):
        name = self.root.get_screen("fields").ids.new_field_input.text.strip()
        tp   = self.root.get_screen("fields").ids.field_type_input.text.strip() or "text"
        ...
        new_fd = {"name": name, "type": tp, "required": False}
