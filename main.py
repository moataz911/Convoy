...
BUILTIN      = ["الاسم", "السن", "رقم الهاتف", "تم"]
...
class ContactApp(MDApp):
    ...
    def __init__(self, **kw):
        ...
        self.fields        = [
            ...
            {"name": "تم",         "type": "checkbox",                       "required": False},
        ]
    ...
    def _open_ftp_browser(self):
        """تصفح ملفات CSV على FTP - مع التحقق من الاتصال أولاً"""
        self._require_ftp(self._execute_ftp_browser)

    def _execute_ftp_browser(self):
        def _bg():
            ftp_c = None
            try:
        ...
        def _bg():
            ...
            try:
                ...
                try:
                    ...
                    Clock.schedule_once(lambda dt: self._snack(f"خطأ في قراءة CSV: {ce}"), 0)
                    return
                ...
                if new_recs:
                    ...
                    Clock.schedule_once(lambda dt: self._snack("الملف فارغ أو بتنسيق غير صحيح"), 0)
            except Exception as e:
    ...
    def _refresh_list(self, append=False):
        ...
        # تحسين: إضافة العناصر بشكل تدريجي عبر Clock لتجنب تجميد الواجهة
    ...
    def _on_drag_end(self, inst, touch):
        if touch.grab_current is inst:
            ...
            if hasattr(self, "_drag_idx") and self._drag_tgt_idx != -1 and self._drag_idx != self._drag_tgt_idx:
                ...
                tgt = max(0, min(self._drag_tgt_idx, len(self.records)))
    ...
    def send_sos(self):
        """إرسال تنبيه SOS فوري"""
        import datetime
        now = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        rec = {"الاسم": "🚨 SOS 🚨", "السن": "URGENT", "رقم الهاتف": now, "تم": "false"}
        # إضافة بقية الحقول كفارغة
        for fd in self.fields:
            if fd["name"] not in rec:
                rec[fd["name"]] = ""
        
        self.records.insert(0, rec)
        if self._save_local_and_sync(immediate_ftp=True):
            self._snack("تم إرسال نداء SOS بنجاح 🚨")
            self._refresh_list()
        else:
            self.records.pop(0)
            self._snack("فشل إرسال SOS - تحقق من الشبكة")
    ...
    def save_record(self):
        ...
        # تعيين قيمة افتراضية لخانة "تم" إذا لم تكن موجودة
        if "تم" not in rec:
            rec["تم"] = "false"
