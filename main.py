    ...
    def _make_field_widget(self, fd, current=""):
        ...
        if ftype == "text":
            ...
            def _on_text(inst, val, w=tf):
                """
                يطبّق ar() في الوقت الفعلي أثناء الكتابة:
                - لو في حروف عربية أصلية (0600-06FF) → استخرج النص الخام وطبّق ar()
                - لو حذف → اقصّر _raw_value بنفس عدد المحروفات المحذوفة
                """
                if w._ar_syncing:
                    return

                has_orig = any('\u0600' <= c <= '\u06FF' for c in val)

                if has_orig:
                    # المستخدم كتب حروف عربية أصلية → استخرجها مع باقي الحروف غير المُعالجة
                    raw = ''.join(
                        c for c in val
                        if not ('\uFE70' <= c <= '\uFEFF')
                        and c not in '\u202D\u202C\u200F\u200E'
                    ...
                    # المستخدم مسح كل النص
                    w._raw_value = ""
                else:
                    # حذف من نص مُعالج (presentation forms فقط)
                    prev_raw = w._raw_value or ""
                    prev_disp = ar(prev_raw) if prev_raw else ""
                    deleted_count = len(prev_disp) - len(val)
                    if deleted_count > 0:
                        new_raw = prev_raw[:-deleted_count]
                        w._raw_value = new_raw
                        new_disp = ar(new_raw) if new_raw else ""
                        if new_disp != val:
                            w._ar_syncing = True
                            w.text = new_disp
                            w._ar_syncing = False
            ...
            # عرض ar() مبدئياً (وضع التعديل: current != "")
            if current:
