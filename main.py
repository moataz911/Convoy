...
class ContactApp(MDApp):
    ...
    def _make_field_widget(self, fd, current=""):
        ...
        if ftype == "text":
            ...
            tf._shape_ev   = None   # guard: prevents duplicate deferred updates

            if fd.get("input_type") == "number":
                # أرقام: لوحة رقمية + فلتر — لا تحتاج تشكيل عربي
                tf.input_filter = "int"
                tf.input_type   = "number"
            else:
                # نص عربي: _on_text يضمن اتصال الحروف وعرضها صحيحاً
                def _on_text(inst, val, w=tf):
                    ...
                    if delta > 0:
                        ...
                        changed = True
                    if changed:
                        # تأجيل عرض التشكيل للـ frame القادم — لا يعطّل الكتابة
                        if w._shape_ev:
                            w._shape_ev.cancel()
            ...
            # وضع التعديل: اعرض النص مشكَّلاً
            if current:
                _is_num = fd.get("input_type") == "number"
                def _init_display(dt, w=tf, c=current, num=_is_num):
                    w._ar_syncing = True
                    w.text = c if num else (_ar_for_input(c) if c else "")
                    w._ar_syncing = False
