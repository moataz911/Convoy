...
class ContactApp(MDApp):
    ...
    def _init_rtl_inputs(self, dt):
        def _mkh(w):
            def h(inst, focused):
                if getattr(w,'_ar_syncing',False): return
                if focused:
                    raw=getattr(w,'_raw_value','') or ''
                    if raw!=w.text:
                        w._ar_syncing=True; w.text=raw; w._ar_syncing=False
                else:
                    raw=w.text.strip(); w._raw_value=raw
                    sh=_ar_for_input(raw) if raw else ''
                    if sh!=w.text:
                        w._ar_syncing=True; w.text=sh; w._ar_syncing=False
            return h
        for sn,fid,hint in [("search","search_input","ابحث بالاسم أو أي معلومة..."),
                             ("fields","new_field_input","اسم الحقل الجديد"),
                             ("fields","field_options_input","خيارات: خيارر1، خيارر2")]:
            try:
                tf=self.root.get_screen(sn).ids[fid]
                tf.hint_text=ar(hint); tf.halign="right"
                tf.base_direction="rtl"; tf.text_language="ar"
                if not getattr(tf,'_ar_focus_bound',False):
                    tf._raw_value=""; tf._ar_syncing=False
                    tf.bind(focus=_mkh(tf)); tf._ar_focus_bound=True
            except Exception as e: print(f"[RTL] {fid}: {e}")
    ...
    def do_search(self, query):
        ...
        for rec in self.records:
            vals = [str(v).lower() for v in rec.values()]
            if not any(q in v or qr in v for v in vals): continue
            parts = []
            if rec.get("السن"):        parts.append(ar(f"السن: {rec['السن']}"))
            if rec.get("رقم الهاتف"): parts.append(ar(f"الهاتف: {rec['رقم الهاتف']}"))
