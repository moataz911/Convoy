...
class ContactApp(MDApp):
    ...
    def _make_field_widget(self, fd, current=""):
        ...
        if ftype == "text":
            ...
            tf.field_type = "text"
            # ═══════════════════════════════════════════════════════
            # لا حاجة لـ _on_text هنا:
            # - base_direction='rtl' يضمن الكتابة من اليمين لليسار
            # - text_language='ar' يخبر Kivy بلغة النص
            # - ar() تُستخدم فقط للـ Labels (عرض)، ليس لحقول الإدخال
            # - الكود القديم كان يفقد الحروف السابقة ويكسر الأرقام
            # ═══════════════════════════════════════════════════════

            # وضع التعديل: اعرض النص الخام مباشرة بدون ar()
            if current:
                def _init_display(dt, w=tf, c=current):
                    w.text = c   # نص خام — لا ar() هنا
    ...
    def _read_val(self, w):
        ...
        if ft == "text":        return w.text.strip()  # مباشر — لا _raw_value
