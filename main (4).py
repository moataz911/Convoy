...
class ContactApp(MDApp):
    ...
    def _save_local_and_sync(self) -> bool:
        """Local-First Helper: حفظ فوري + FTP في خلفية."""
        try:
            buf = io.StringIO()
    ...
    def save_record(self):
        ...
        for fd in self.fields:
            ...
            rec[fd["name"]] = val
        def _do():
            Clock.schedule_once(lambda dt: self._snack("جاري الحفظ..."), 0)
            self.records.insert(0, rec); self._dirty = True
            if self._save_local_and_sync():
                self._dirty = False
                Clock.schedule_once(lambda dt: self._snack("تمت الإضافة بنجاح ✓"), 0)
                Clock.schedule_once(lambda dt: self._build_add_form(), 0)
            else:
                self.records.pop(0); self._dirty = False
                Clock.schedule_once(lambda dt: self._no_ftp_dlg("فشل حفظ البيانات"), 0)
        self._require_ftp(lambda: threading.Thread(target=_do, daemon=True).start())
        ...
        def _do(x):
            ...
            def _bg():
                ...
                if self._save_local_and_sync():
                    ...
                    self.records.insert(0, rec_to_delete); self._dirty = False
                    Clock.schedule_once(lambda dt: self._no_ftp_dlg("فشل حذف البيانات"), 0)
            threading.Thread(target=_bg, daemon=True).start()
