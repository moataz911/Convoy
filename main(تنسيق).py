...
"""
import io, csv, json, os, re, threading, sys, time

# ── Plyer — إشعارات Android ──────────────────────────────────────────
try:
...
class _RetryNeeded(Exception):
    """داخلي فقط — يُشير إلى أن العملية تستحق إعادة المحاولة."""
...
class FTPManager:
    ...
    def read_csv_by_name(self, filename, max_retries=2, retry_delay=1.0):
        """
        يقرأ ملف CSV محدد من FTP مع إعادة المحاولة عند الفشل.
        max_retries=2 → يحاول 3 مرات إجمالاً قبل الاستسلام.
        حالات تستدعي إعادة المحاولة:
          - ملف فارغ تماماً (raw == b"")  → race condition محتمل
          - خطأ في parsing الـ CSV         → ملف مكتوب جزئياً
          - خطأ في الاتصال               → انقطاع مؤقت
        ملف صحيح بدون سجلات (header فقط) لا يستدعي retry.
        """
        for attempt in range(max_retries + 1):
            if attempt > 0:
                print(f"[FTP Retry {attempt}/{max_retries}] {filename}")
            ...
            try:
                ...
                # ملف فارغ تماماً → يستحق retry (race condition محتمل)
                if not raw:
                    raise _RetryNeeded(f"empty file: {filename}")
                ...
                try:
                    ...
                    # CSV تالف → يستحق retry
                    raise _RetryNeeded(f"broken CSV: {filename}")

                # نجح — أعِد النتيجة (حتى لو فارغة بشكل طبيعي)
                return records

            except _RetryNeeded as e:
                print(f"[FTP Read] attempt {attempt+1}/{max_retries+1}: {e}")
                if attempt == max_retries:
                    print(f"[FTP Read] giving up on {filename}")
                    return []
            except Exception as e:
                print(f"[FTP Read {filename}] attempt {attempt+1}: {e}")
                if attempt == max_retries:
                    return []
            finally:
                if ftp: self._close(ftp)
...
class ContactApp(MDApp):
    ...
    def _render_tabs(self):
        try:
            ...
            for fname in self.files_data.keys():
                ...
                # فحص وجود SOS في هذا الملف
                has_sos = any(
                    "SOS" in str(v)
                    for rec in self.files_data.get(fname, [])
                    for v in rec.values()
                )
                if is_active and has_sos:
                    btn = MDRaisedButton(
                        text=fname, size_hint=(None, 1), width=dp(140),
                        md_bg_color=(0.90, 0.10, 0.10, 1),   # أحمر — نشط + SOS
                        theme_text_color="Custom",
                        text_color=(1, 1, 1, 1),
                    )
                elif is_active:
                    ...
                    btn = MDFlatButton(
                        text=fname, size_hint=(None, 1), width=dp(140),
                        theme_text_color="Custom",
                        text_color=(1, 0.35, 0.35, 1),       # أحمر فاتح — غير نشط + SOS
                    )
                else:
                    btn = MDFlatButton(
                        text=fname, size_hint=(None, 1), width=dp(140),
    ...
    # ── SOS Detection & Notification ──────────────────────────
    def _check_and_notify_sos(self, filename, records):
        """
        يفحص الملف بحثاً عن كلمة SOS في أي مكان.
        - يُضيف للـ _sos_active_files إن وُجدت SOS
        - يُرسل إشعار نظام مرة واحدة فقط لكل ملف
        - عند اختفاء SOS يُعيد ضبط جميع الحالات
        """
        has_sos = any(
            "SOS" in str(v)
            for rec in records
        ...
        if has_sos:
            self._sos_active_files.add(filename)
            if filename not in self._sos_notified:
                # ملف جديد به SOS — أرسل إشعار وأعِد تفعيل الشريط
                self._sos_notified.add(filename)
                self._sos_dismissed.discard(filename)
                self._send_sos_notification(filename)
        else:
            # SOS اختفت — إعادة ضبط كامل للملف
            self._sos_active_files.discard(filename)
            self._sos_notified.discard(filename)
            self._sos_dismissed.discard(filename)

    def _update_sos_bar(self):
        """يحدّث شريط SOS في الشاشة الرئيسية (يظهر/يختفي حسب الحالة)."""
        try:
            bar = self.root.get_screen("home").ids.sos_alert_bar
            lbl = self.root.get_screen("home").ids.sos_alert_label
            # الملفات النشطة التي لم يُقفل شريطها
            active_sos = self._sos_active_files - self._sos_dismissed
            if active_sos:
                files_text = "\u060c  ".join(sorted(active_sos))   # فاصلة عربية
                lbl.text   = ar(f"SOS \u2014 {files_text}")        # — em dash
                bar.height = dp(48)
            else:
                lbl.text   = ""
                bar.height = 0
    ...
    def dismiss_sos(self):
        """يُقفل شريط SOS عند ضغط المستخدم على زر الإغلاق ✕."""
        self._sos_dismissed.update(self._sos_active_files)
        self._update_sos_bar()

    def _send_sos_notification(self, filename):
        """يُرسل إشعار Android — fallback للـ snackbar لو plyer غير متوفر."""
        title   = "SOS"
        message = f"SOS \u2014 {filename}"   # SOS — filename
        if _PLYER_OK:
            try:
                _plyer_notify.notify(
                    title=title,
                    message=message,
                    app_name="\u0642\u0648\u0627\u0641\u0644 \u0645\u0631\u0633\u0627\u0644",
                    timeout=0,   # 0 = لا يختفي تلقائياً
                )
                print(f"[SOS] Notification sent: {filename}")
            except Exception as e:
                print(f"[SOS] Notification failed: {e}")
                self._snack(message)
        else:
            # fallback: snackbar داخل التطبيق
            self._snack(message)
