    ...
    def show_ftp_settings(self, instance):
        ...
        def test_ftp(btn):
            try:
                ...
                # Test connection in a separate thread and schedule UI updates
                # on Kivy's main thread. Lambdas cannot contain assignments, so
                # use a helper callback that sets the status label text.
                def _set_status(message):
                    self.status_label.text = ar(message)
                ...
                def _test_connection():
                    try:
                        ...
                        ftp.cwd(ftp_dir)
                        ftp.quit()
                        Clock.schedule_once(lambda dt: _set_status("✓ اتصال FTP ناجح"), 0)
                    except Exception as e:
                        error_message = f"✗ فشل اتصال FTP: {e}"
                        Clock.schedule_once(lambda dt: _set_status(error_message), 0)
